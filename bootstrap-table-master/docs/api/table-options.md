  ...
  **Hint:** Instead of inline functions you also can use function names.
  ...
  ``` javascript 
  {
    btnRemoveEvenRows: {
      ...
      'attributes': {
        'title': 'Remove all data which has a even id'
      }
    }
  }
  ```
...
- **Detail:**
  ...
  Set `true` to select checkbox or radiobox when clicking data.
...
- **Detail:**
  ...
  Key in incoming json containing `'data'` data list.

- **Default:** `'data'`

- **Example:** [Total/Data Field](https://examples.bootstrap-table.com/#options/count-data-field.html)
...
- **Detail:**
  ...
  Enable expansion per row when `detailView` is set to `true`. Return true and the row will be enabled for expansion, return false and expansion for the row will be disabled. Default function returns true to enable expansion for all data.
...
- **Detail:**
  ...
  Set `true` to enable the multiple selection row. Can use the ctrl+click to select one row or use shift+click to select a range of data.
...
- **Detail:**
  ...
  *Hint: If the table has lesser data as the option(s), the options will be hidden automatically, to disable that feature you can set [smartDisplay](https://bootstrap-table.com/docs/api/table-options/#smartdisplay) to `false`*
...
- **Detail:**
  ...
  * `pageInfo` Shows which dataset will be displayed on the current page (e.g. `Showing 1 to 10 of 54 data`).
  * `pageInfoShort` Similar to `pageInfo`, but it only displays how much data the table has (e.g. `Showing 54 data`).
  * `pageSize` Shows the dropdown which defines how many data should be displayed on the page.
...
- **Detail:**
  ...
  Set `true` to show a extended version of pagination (including the count of all data with out filters).
...
- **Attribute:** `data-count-field`

- **Type:** `String`

- **Detail:**

  Key in incoming json containing `'count'` data.
...
- **Attribute:** `data-count-not-filtered`
...
- **Attribute:** `data-count-not-filtered-field`
...
- **Example:** [Total Not Filtered Field](https://examples.bootstrap-table.com/#options/count-not-filtered-field.html)
