...
- **Update:** Fixed not update selected data bug when using `checkAll`/`uncheckAll`.
...
- **Update:** Fixed `updateByUniqueId` method cannot update multiple data bug.
...
- **Update:** Fixed `showRow` method show all hidden data bug.
...
- **Update(copy-data):** Fixed copy data bug with fixed-column.
...
- **Update(reorder-data):** Fixed `this` context of `onPostBody` error.
...
- **New(reorder-data):** Added `search` and `cardView` supported.
- **New(sticky-header):** Added support for all themes.
- **New(toolbar):** Added support for all themes.
- **New(reorder-data):** Added `search` and `cardView` support.
...
- **Update:** Fixed treegrid duplicate data bug.
...
- **New:** Improved `scrollTo` method to allow `data` units.
...
- **Update(export extension):** Fixed export all data to pdf bug.
...
- **Update(js):** Fixed sort column shows hidden data in `server` side pagination bug.
...
- fix(filter-control extension): fix #4008, select all data and `keyup` event error
...
- fix(export extension): fix #2220, selected data does not work when data-pagination-side is server
- fix(reorder-row extension): fix #1343, reorder data bug with pagination
...
- fix(js): fix sorting data not working bug
...
- [bug] Fix #970: `click`and `dblclick` bug on no-data table.
...
- [enh] Added key-events, mobile, filter-control, reorder-columns, reorder-data, resizable, natural-sorting, toolbar extensions, and update the extensions name.
...
- [enh] Added 'getAllSelections' method to get checked data across all pages.
...
- [bug] Fix row state field value bug when there are disabled data.
